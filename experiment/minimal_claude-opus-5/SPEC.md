...
Python 3.12. Use `uv`. Third party packages are allowed and the package feed is already
configured on this machine, so `uv add` and `uv run` work without extra flags.
